...
import net.julio12318.siivagunnermusicdiscmod.block.entity.custom.HalationJukeboxBlockEntity;
...
public class HalationJukeboxBlock extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<HalationJukeboxBlock> CODEC = HalationJukeboxBlock.createCodec(HalationJukeboxBlock::new);
    public static final BooleanProperty HAS_RECORD;

    public HalationJukeboxBlock(Settings settings) {
        super(settings);
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new HalationJukeboxBlockEntity(pos, state);
    }
    ...
    protected void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (!state.isOf(newState.getBlock())) {
            ...
            if (var7 instanceof HalationJukeboxBlockEntity halationJukeboxBlockEntity) {
                halationJukeboxBlockEntity.dropRecord();
            }
    ...
    public static ItemActionResult tryPlayStack(World world, BlockPos pos, ItemStack stack, PlayerEntity player) {
        ...
        if (jukeboxPlayableComponent == null) {
            ...
            if (blockState.isOf(ModBlocks.HALATION_JUKEBOX) && !(Boolean)blockState.get(HalationJukeboxBlock.HAS_RECORD)) {
                if (!world.isClient) {
                    ItemStack itemStack = stack.splitUnlessCreative(1, player);
                    BlockEntity var8 = world.getBlockEntity(pos);
                    if (var8 instanceof HalationJukeboxBlockEntity halationJukeboxBlockEntity) {
                        halationJukeboxBlockEntity.setStack(itemStack);
    ...
    public int getWeakRedstonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        ...
        if (var6 instanceof HalationJukeboxBlockEntity halationJukeboxBlockEntity) {
            if (halationJukeboxBlockEntity.getManager().isPlaying()) {
                return 15;
            }
        }
    ...
    protected int getComparatorOutput(BlockState state, World world, BlockPos pos) {
        ...
        if (var5 instanceof HalationJukeboxBlockEntity halationJukeboxBlockEntity) {
            return halationJukeboxBlockEntity.getComparatorOutput();
        } else {
            return 0;
        }
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return state.get(HAS_RECORD) ? validateTicker(type, ModBlockEntities.HALATION_JUKEBOX_E, HalationJukeboxBlockEntity::tick) : null;
    }
