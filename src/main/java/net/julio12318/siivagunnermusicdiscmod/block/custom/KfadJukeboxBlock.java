...
import net.julio12318.siivagunnermusicdiscmod.block.entity.custom.KfadJukeboxBlockEntity;
...
public class KfadJukeboxBlock extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<KfadJukeboxBlock> CODEC = KfadJukeboxBlock.createCodec(KfadJukeboxBlock::new);
    public static final BooleanProperty HAS_RECORD;

    public KfadJukeboxBlock(Settings settings) {
        super(settings);
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new KfadJukeboxBlockEntity(pos, state);
    }
    ...
    protected void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (!state.isOf(newState.getBlock())) {
            ...
            if (var7 instanceof KfadJukeboxBlockEntity kfadJukeboxBlockEntity) {
                kfadJukeboxBlockEntity.dropRecord();
            }
    ...
    public static ItemActionResult tryPlayStack(World world, BlockPos pos, ItemStack stack, PlayerEntity player) {
        ...
        if (jukeboxPlayableComponent == null) {
            ...
            if (blockState.isOf(ModBlocks.KFAD_JUKEBOX) && !(Boolean)blockState.get(KfadJukeboxBlock.HAS_RECORD)) {
                if (!world.isClient) {
                    ItemStack itemStack = stack.splitUnlessCreative(1, player);
                    BlockEntity var8 = world.getBlockEntity(pos);
                    if (var8 instanceof KfadJukeboxBlockEntity kfadJukeboxBlockEntity) {
                        kfadJukeboxBlockEntity.setStack(itemStack);
    ...
    public int getWeakRedstonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        ...
        if (var6 instanceof KfadJukeboxBlockEntity kfadJukeboxBlockEntity) {
            if (kfadJukeboxBlockEntity.getManager().isPlaying()) {
                return 15;
            }
        }
    ...
    protected int getComparatorOutput(BlockState state, World world, BlockPos pos) {
        ...
        if (var5 instanceof KfadJukeboxBlockEntity kfadJukeboxBlockEntity) {
            return kfadJukeboxBlockEntity.getComparatorOutput();
        } else {
            return 0;
        }
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return state.get(HAS_RECORD) ? validateTicker(type, ModBlockEntities.KFAD_JUKEBOX_E, KfadJukeboxBlockEntity::tick) : null;
    }
