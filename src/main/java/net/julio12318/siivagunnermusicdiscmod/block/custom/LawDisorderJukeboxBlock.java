...
import net.julio12318.siivagunnermusicdiscmod.block.entity.custom.LawDisorderJukeboxBlockEntity;
...
public class LawDisorderJukeboxBlock extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<LawDisorderJukeboxBlock> CODEC = LawDisorderJukeboxBlock.createCodec(LawDisorderJukeboxBlock::new);
    public static final BooleanProperty HAS_RECORD;

    public LawDisorderJukeboxBlock(Settings settings) {
        super(settings);
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new LawDisorderJukeboxBlockEntity(pos, state);
    }
    ...
    protected void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (!state.isOf(newState.getBlock())) {
            ...
            if (var7 instanceof LawDisorderJukeboxBlockEntity lawDisorderJukeboxBlockEntity) {
                lawDisorderJukeboxBlockEntity.dropRecord();
            }
    ...
    public static ItemActionResult tryPlayStack(World world, BlockPos pos, ItemStack stack, PlayerEntity player) {
        ...
        if (jukeboxPlayableComponent == null) {
            ...
            if (blockState.isOf(ModBlocks.LAW_DISORDER_JUKEBOX) && !(Boolean)blockState.get(LawDisorderJukeboxBlock.HAS_RECORD)) {
                if (!world.isClient) {
                    ItemStack itemStack = stack.splitUnlessCreative(1, player);
                    BlockEntity var8 = world.getBlockEntity(pos);
                    if (var8 instanceof LawDisorderJukeboxBlockEntity lawDisorderJukeboxBlockEntity) {
                        lawDisorderJukeboxBlockEntity.setStack(itemStack);
    ...
    public int getWeakRedstonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        ...
        if (var6 instanceof LawDisorderJukeboxBlockEntity lawDisorderJukeboxBlockEntity) {
            if (lawDisorderJukeboxBlockEntity.getManager().isPlaying()) {
                return 15;
            }
        }
    ...
    protected int getComparatorOutput(BlockState state, World world, BlockPos pos) {
        ...
        if (var5 instanceof LawDisorderJukeboxBlockEntity lawDisorderJukeboxBlockEntity) {
            return lawDisorderJukeboxBlockEntity.getComparatorOutput();
        } else {
            return 0;
        }
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return state.get(HAS_RECORD) ? validateTicker(type, ModBlockEntities.LAW_DISORDER_JUKEBOX_E, LawDisorderJukeboxBlockEntity::tick) : null;
    }
