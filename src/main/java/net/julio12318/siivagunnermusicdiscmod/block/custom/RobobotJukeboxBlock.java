...
import net.julio12318.siivagunnermusicdiscmod.block.entity.custom.RobobotJukeboxBlockEntity;
...
public class RobobotJukeboxBlock extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<RobobotJukeboxBlock> CODEC = RobobotJukeboxBlock.createCodec(RobobotJukeboxBlock::new);
    public static final BooleanProperty HAS_RECORD;

    public RobobotJukeboxBlock(Settings settings) {
        super(settings);
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new RobobotJukeboxBlockEntity(pos, state);
    }
    ...
    protected void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (!state.isOf(newState.getBlock())) {
            ...
            if (var7 instanceof RobobotJukeboxBlockEntity robobotJukeboxBlockEntity) {
                robobotJukeboxBlockEntity.dropRecord();
            }
    ...
    public static ItemActionResult tryPlayStack(World world, BlockPos pos, ItemStack stack, PlayerEntity player) {
        ...
        if (jukeboxPlayableComponent == null) {
            ...
            if (blockState.isOf(ModBlocks.ROBOBOT_JUKEBOX) && !(Boolean)blockState.get(RobobotJukeboxBlock.HAS_RECORD)) {
                if (!world.isClient) {
                    ItemStack itemStack = stack.splitUnlessCreative(1, player);
                    BlockEntity var8 = world.getBlockEntity(pos);
                    if (var8 instanceof RobobotJukeboxBlockEntity robobotJukeboxBlockEntity) {
                        robobotJukeboxBlockEntity.setStack(itemStack);
    ...
    public int getWeakRedstonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        ...
        if (var6 instanceof RobobotJukeboxBlockEntity robobotJukeboxBlockEntity) {
            if (robobotJukeboxBlockEntity.getManager().isPlaying()) {
                return 15;
            }
        }
    ...
    protected int getComparatorOutput(BlockState state, World world, BlockPos pos) {
        ...
        if (var5 instanceof RobobotJukeboxBlockEntity robobotJukeboxBlockEntity) {
            return robobotJukeboxBlockEntity.getComparatorOutput();
        } else {
            return 0;
        }
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return state.get(HAS_RECORD) ? validateTicker(type, ModBlockEntities.ROBOBOT_JUKEBOX_E, RobobotJukeboxBlockEntity::tick) : null;
    }
