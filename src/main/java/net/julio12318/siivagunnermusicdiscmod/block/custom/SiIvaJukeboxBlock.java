...
import net.julio12318.siivagunnermusicdiscmod.block.entity.custom.SiIvaJukeboxBlockEntity;
...
public class SiIvaJukeboxBlock extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<SiIvaJukeboxBlock> CODEC = SiIvaJukeboxBlock.createCodec(SiIvaJukeboxBlock::new);
    public static final BooleanProperty HAS_RECORD;

    public SiIvaJukeboxBlock(Settings settings) {
        super(settings);
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new SiIvaJukeboxBlockEntity(pos, state);
    }
    ...
    protected void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (!state.isOf(newState.getBlock())) {
            ...
            if (var7 instanceof SiIvaJukeboxBlockEntity siivaJukeboxBlockEntity) {
                siivaJukeboxBlockEntity.dropRecord();
            }
    ...
    public static ItemActionResult tryPlayStack(World world, BlockPos pos, ItemStack stack, PlayerEntity player) {
        ...
        if (jukeboxPlayableComponent == null) {
            ...
            if (blockState.isOf(ModBlocks.SIIVA_JUKEBOX) && !(Boolean)blockState.get(SiIvaJukeboxBlock.HAS_RECORD)) {
                if (!world.isClient) {
                    ItemStack itemStack = stack.splitUnlessCreative(1, player);
                    BlockEntity var8 = world.getBlockEntity(pos);
                    if (var8 instanceof SiIvaJukeboxBlockEntity siIvaJukeboxBlockEntity) {
                        siIvaJukeboxBlockEntity.setStack(itemStack);
    ...
    public int getWeakRedstonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        ...
        if (var6 instanceof SiIvaJukeboxBlockEntity siivaJukeboxBlockEntity) {
            if (siivaJukeboxBlockEntity.getManager().isPlaying()) {
                return 15;
            }
        }
    ...
    protected int getComparatorOutput(BlockState state, World world, BlockPos pos) {
        ...
        if (var5 instanceof SiIvaJukeboxBlockEntity siivaJukeboxBlockEntity) {
            return siivaJukeboxBlockEntity.getComparatorOutput();
        } else {
            return 0;
        }
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return state.get(HAS_RECORD) ? validateTicker(type, ModBlockEntities.SIIVA_JUKEBOX_E, SiIvaJukeboxBlockEntity::tick) : null;
    }
