...
import net.julio12318.siivagunnermusicdiscmod.block.entity.custom.SorrizoJukeboxBlockEntity;
...
public class SorrizoJukeboxBlock extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<SorrizoJukeboxBlock> CODEC = SorrizoJukeboxBlock.createCodec(SorrizoJukeboxBlock::new);
    public static final BooleanProperty HAS_RECORD;

    public SorrizoJukeboxBlock(Settings settings) {
        super(settings);
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new SorrizoJukeboxBlockEntity(pos, state);
    }
    ...
    protected void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (!state.isOf(newState.getBlock())) {
            ...
            if (var7 instanceof SorrizoJukeboxBlockEntity sorrizoJukeboxBlockEntity) {
                sorrizoJukeboxBlockEntity.dropRecord();
            }
    ...
    public static ItemActionResult tryPlayStack(World world, BlockPos pos, ItemStack stack, PlayerEntity player) {
        ...
        if (jukeboxPlayableComponent == null) {
            ...
            if (blockState.isOf(ModBlocks.SORRIZO_JUKEBOX) && !(Boolean)blockState.get(SorrizoJukeboxBlock.HAS_RECORD)) {
                if (!world.isClient) {
                    ItemStack itemStack = stack.splitUnlessCreative(1, player);
                    BlockEntity var8 = world.getBlockEntity(pos);
                    if (var8 instanceof SorrizoJukeboxBlockEntity sorrizoJukeboxBlockEntity) {
                        sorrizoJukeboxBlockEntity.setStack(itemStack);
    ...
    public int getWeakRedstonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        ...
        if (var6 instanceof SorrizoJukeboxBlockEntity sorrizoJukeboxBlockEntity) {
            if (sorrizoJukeboxBlockEntity.getManager().isPlaying()) {
                return 15;
            }
        }
    ...
    protected int getComparatorOutput(BlockState state, World world, BlockPos pos) {
        ...
        if (var5 instanceof SorrizoJukeboxBlockEntity sorrizoJukeboxBlockEntity) {
            return sorrizoJukeboxBlockEntity.getComparatorOutput();
        } else {
            return 0;
        }
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return state.get(HAS_RECORD) ? validateTicker(type, ModBlockEntities.SORRIZO_JUKEBOX_E, SorrizoJukeboxBlockEntity::tick) : null;
    }
