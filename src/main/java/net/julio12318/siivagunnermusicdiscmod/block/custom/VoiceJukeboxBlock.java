...
import net.julio12318.siivagunnermusicdiscmod.block.entity.custom.VoiceJukeboxBlockEntity;
...
public class VoiceJukeboxBlock extends BlockWithEntity implements BlockEntityProvider {
    public static final MapCodec<VoiceJukeboxBlock> CODEC = VoiceJukeboxBlock.createCodec(VoiceJukeboxBlock::new);
    public static final BooleanProperty HAS_RECORD;

    public VoiceJukeboxBlock(Settings settings) {
        super(settings);
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new VoiceJukeboxBlockEntity(pos, state);
    }
    ...
    protected void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (!state.isOf(newState.getBlock())) {
            ...
            if (var7 instanceof VoiceJukeboxBlockEntity voiceJukeboxBlockEntity) {
                voiceJukeboxBlockEntity.dropRecord();
            }
    ...
    public static ItemActionResult tryPlayStack(World world, BlockPos pos, ItemStack stack, PlayerEntity player) {
        ...
        if (jukeboxPlayableComponent == null) {
            ...
            if (blockState.isOf(ModBlocks.VOICE_JUKEBOX) && !(Boolean)blockState.get(VoiceJukeboxBlock.HAS_RECORD)) {
                if (!world.isClient) {
                    ItemStack itemStack = stack.splitUnlessCreative(1, player);
                    BlockEntity var8 = world.getBlockEntity(pos);
                    if (var8 instanceof VoiceJukeboxBlockEntity voiceJukeboxBlockEntity) {
                        voiceJukeboxBlockEntity.setStack(itemStack);
    ...
    public int getWeakRedstonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        ...
        if (var6 instanceof VoiceJukeboxBlockEntity voiceJukeboxBlockEntity) {
            if (voiceJukeboxBlockEntity.getManager().isPlaying()) {
                return 15;
            }
        }
    ...
    protected int getComparatorOutput(BlockState state, World world, BlockPos pos) {
        ...
        if (var5 instanceof VoiceJukeboxBlockEntity voiceJukeboxBlockEntity) {
            return voiceJukeboxBlockEntity.getComparatorOutput();
        } else {
            return 0;
        }
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return state.get(HAS_RECORD) ? validateTicker(type, ModBlockEntities.VOICE_JUKEBOX_E, VoiceJukeboxBlockEntity::tick) : null;
    }
