...
import net.julio12318.siivagunnermusicdiscmod.block.jukebox.HalationJukeboxManager;
...
public class HalationJukeboxBlockEntity extends BlockEntity implements Clearable, SingleStackInventory.SingleStackBlockEntityInventory{
    ...
    private final HalationJukeboxManager manager;


    public HalationJukeboxBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.HALATION_JUKEBOX_E, pos, state);
        this.recordStack = ItemStack.EMPTY;
        this.manager = new HalationJukeboxManager(this::onManagerChange, this.getPos());
    }


    public HalationJukeboxManager getManager() {
        return this.manager;
    }
    ...
    private void onRecordStackChanged(boolean hasRecord) {
        if (this.world != null && this.world.getBlockState(this.getPos()) == this.getCachedState()) {
            this.world.setBlockState(this.getPos(), (BlockState)this.getCachedState().with(HalationJukeboxBlock.HAS_RECORD, hasRecord), 2);
    ...
    public static void tick(World world, BlockPos pos, BlockState state, HalationJukeboxBlockEntity blockEntity) {
