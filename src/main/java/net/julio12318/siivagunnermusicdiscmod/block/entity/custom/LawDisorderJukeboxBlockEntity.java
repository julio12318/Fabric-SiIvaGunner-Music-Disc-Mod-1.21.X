...
import net.julio12318.siivagunnermusicdiscmod.block.jukebox.LawDisorderJukeboxManager;
...
public class LawDisorderJukeboxBlockEntity extends BlockEntity implements Clearable, SingleStackInventory.SingleStackBlockEntityInventory{
    ...
    private final LawDisorderJukeboxManager manager;


    public LawDisorderJukeboxBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.LAW_DISORDER_JUKEBOX_E, pos, state);
        this.recordStack = ItemStack.EMPTY;
        this.manager = new LawDisorderJukeboxManager(this::onManagerChange, this.getPos());
    }


    public LawDisorderJukeboxManager getManager() {
        return this.manager;
    }
    ...
    private void onRecordStackChanged(boolean hasRecord) {
        if (this.world != null && this.world.getBlockState(this.getPos()) == this.getCachedState()) {
            this.world.setBlockState(this.getPos(), (BlockState)this.getCachedState().with(LawDisorderJukeboxBlock.HAS_RECORD, hasRecord), 2);
    ...
    public static void tick(World world, BlockPos pos, BlockState state, LawDisorderJukeboxBlockEntity blockEntity) {
