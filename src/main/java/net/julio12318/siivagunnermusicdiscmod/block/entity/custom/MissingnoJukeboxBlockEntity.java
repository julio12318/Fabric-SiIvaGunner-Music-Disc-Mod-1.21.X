...
import net.julio12318.siivagunnermusicdiscmod.block.custom.MissingnoJukeboxBlock;
import net.julio12318.siivagunnermusicdiscmod.block.entity.ModBlockEntities;
import net.julio12318.siivagunnermusicdiscmod.block.jukebox.MissingnoJukeboxManager;
...
public class MissingnoJukeboxBlockEntity extends BlockEntity implements Clearable, SingleStackInventory.SingleStackBlockEntityInventory{
    ...
    private final MissingnoJukeboxManager manager;


    public MissingnoJukeboxBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.MISSINGNO_JUKEBOX_E, pos, state);
        this.recordStack = ItemStack.EMPTY;
        this.manager = new MissingnoJukeboxManager(this::onManagerChange, this.getPos());
    }


    public MissingnoJukeboxManager getManager() {
        return this.manager;
    }
    ...
    private void onRecordStackChanged(boolean hasRecord) {
        if (this.world != null && this.world.getBlockState(this.getPos()) == this.getCachedState()) {
            this.world.setBlockState(this.getPos(), (BlockState)this.getCachedState().with(MissingnoJukeboxBlock.HAS_RECORD, hasRecord), 2);
    ...
    public static void tick(World world, BlockPos pos, BlockState state, MissingnoJukeboxBlockEntity blockEntity) {
