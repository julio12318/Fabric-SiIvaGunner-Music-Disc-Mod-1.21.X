...
import net.julio12318.siivagunnermusicdiscmod.block.jukebox.RobobotJukeboxManager;
...
public class RobobotJukeboxBlockEntity extends BlockEntity implements Clearable, SingleStackInventory.SingleStackBlockEntityInventory{
    ...
    private final RobobotJukeboxManager manager;


    public RobobotJukeboxBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.ROBOBOT_JUKEBOX_E, pos, state);
        this.recordStack = ItemStack.EMPTY;
        this.manager = new RobobotJukeboxManager(this::onManagerChange, this.getPos());
    }


    public RobobotJukeboxManager getManager() {
        return this.manager;
    }
    ...
    private void onRecordStackChanged(boolean hasRecord) {
        if (this.world != null && this.world.getBlockState(this.getPos()) == this.getCachedState()) {
            this.world.setBlockState(this.getPos(), (BlockState)this.getCachedState().with(RobobotJukeboxBlock.HAS_RECORD, hasRecord), 2);
    ...
    public static void tick(World world, BlockPos pos, BlockState state, RobobotJukeboxBlockEntity blockEntity) {
