...
import net.julio12318.siivagunnermusicdiscmod.block.custom.SiIvaJukeboxBlock;
import net.julio12318.siivagunnermusicdiscmod.block.entity.ModBlockEntities;
import net.julio12318.siivagunnermusicdiscmod.block.jukebox.SiIvaJukeboxManager;
...
public class SiIvaJukeboxBlockEntity extends BlockEntity implements Clearable, SingleStackInventory.SingleStackBlockEntityInventory{
    ...
    private final SiIvaJukeboxManager manager;


    public SiIvaJukeboxBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.SIIVA_JUKEBOX_E, pos, state);
        this.recordStack = ItemStack.EMPTY;
        this.manager = new SiIvaJukeboxManager(this::onManagerChange, this.getPos());
    }


    public SiIvaJukeboxManager getManager() {
        return this.manager;
    }
    ...
    private void onRecordStackChanged(boolean hasRecord) {
        if (this.world != null && this.world.getBlockState(this.getPos()) == this.getCachedState()) {
            this.world.setBlockState(this.getPos(), (BlockState)this.getCachedState().with(SiIvaJukeboxBlock.HAS_RECORD, hasRecord), 2);
    ...
    public static void tick(World world, BlockPos pos, BlockState state, SiIvaJukeboxBlockEntity blockEntity) {
