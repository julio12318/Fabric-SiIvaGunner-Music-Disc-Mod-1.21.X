...
import net.julio12318.siivagunnermusicdiscmod.block.jukebox.SorrizoJukeboxManager;
...
public class SorrizoJukeboxBlockEntity extends BlockEntity implements Clearable, SingleStackInventory.SingleStackBlockEntityInventory{
    ...
    private final SorrizoJukeboxManager manager;


    public SorrizoJukeboxBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.SORRIZO_JUKEBOX_E, pos, state);
        this.recordStack = ItemStack.EMPTY;
        this.manager = new SorrizoJukeboxManager(this::onManagerChange, this.getPos());
    }


    public SorrizoJukeboxManager getManager() {
        return this.manager;
    }
    ...
    private void onRecordStackChanged(boolean hasRecord) {
        if (this.world != null && this.world.getBlockState(this.getPos()) == this.getCachedState()) {
            this.world.setBlockState(this.getPos(), (BlockState)this.getCachedState().with(SorrizoJukeboxBlock.HAS_RECORD, hasRecord), 2);
    ...
    public static void tick(World world, BlockPos pos, BlockState state, SorrizoJukeboxBlockEntity blockEntity) {
