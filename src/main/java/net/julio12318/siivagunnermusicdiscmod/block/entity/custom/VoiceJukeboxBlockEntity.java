...
import net.julio12318.siivagunnermusicdiscmod.block.custom.VoiceJukeboxBlock;
import net.julio12318.siivagunnermusicdiscmod.block.entity.ModBlockEntities;
import net.julio12318.siivagunnermusicdiscmod.block.jukebox.VoiceJukeboxManager;
...
public class VoiceJukeboxBlockEntity extends BlockEntity implements Clearable, SingleStackInventory.SingleStackBlockEntityInventory{
    ...
    private final VoiceJukeboxManager manager;


    public VoiceJukeboxBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.VOICE_JUKEBOX_E, pos, state);
        this.recordStack = ItemStack.EMPTY;
        this.manager = new VoiceJukeboxManager(this::onManagerChange, this.getPos());
    }


    public VoiceJukeboxManager getManager() {
        return this.manager;
    }
    ...
    private void onRecordStackChanged(boolean hasRecord) {
        if (this.world != null && this.world.getBlockState(this.getPos()) == this.getCachedState()) {
            this.world.setBlockState(this.getPos(), (BlockState)this.getCachedState().with(VoiceJukeboxBlock.HAS_RECORD, hasRecord), 2);
    ...
    public static void tick(World world, BlockPos pos, BlockState state, VoiceJukeboxBlockEntity blockEntity) {
