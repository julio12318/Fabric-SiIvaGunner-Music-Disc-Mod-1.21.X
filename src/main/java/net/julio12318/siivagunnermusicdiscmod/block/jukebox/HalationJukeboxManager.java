...
public class HalationJukeboxManager {
    public static final int TICKS_PER_SECOND = 20;
    private long ticksSinceSongStarted;
    @Nullable
    private RegistryEntry<JukeboxSong> song;
    private final BlockPos pos;
    private final HalationJukeboxManager.ChangeNotifier changeNotifier;

    public HalationJukeboxManager(HalationJukeboxManager.ChangeNotifier changeNotifier, BlockPos pos) {
    ...
    private static void spawnNoteParticles(WorldAccess world, BlockPos pos) {
        if (world instanceof ServerWorld serverWorld) {
            ...
            serverWorld.spawnParticles(ModParticles.WINTERFES_FLAKE_PARTICLE, vec3d.getX(), vec3d.getY(), vec3d.getZ(), 0, f, 0.0F, 0.0F, 1.0F);
        }
