...
public class RobobotJukeboxManager {
    public static final int TICKS_PER_SECOND = 20;
    private long ticksSinceSongStarted;
    @Nullable
    private RegistryEntry<JukeboxSong> song;
    private final BlockPos pos;
    private final RobobotJukeboxManager.ChangeNotifier changeNotifier;

    public RobobotJukeboxManager(RobobotJukeboxManager.ChangeNotifier changeNotifier, BlockPos pos) {
    ...
    private static void spawnNoteParticles(WorldAccess world, BlockPos pos) {
        if (world instanceof ServerWorld serverWorld) {
            ...
            int part = world.getRandom().nextInt(2);
            if (part == 0) {
                serverWorld.spawnParticles(ModParticles.BLUE_NOTE_PARTICLE, vec3d.getX(), vec3d.getY(), vec3d.getZ(), 0, f, 0.0F, 0.0F, 1.0F);
            }
            else{
                serverWorld.spawnParticles(ModParticles.GREEN_NOTE_PARTICLE, vec3d.getX(), vec3d.getY(), vec3d.getZ(), 0, f, 0.0F, 0.0F, 1.0F);
            }
        }
