package net.julio12318.siivagunnermusicdiscmod.block.jukebox;
...
public class SorrizoJukeboxManager {
    public static final int TICKS_PER_SECOND = 20;
    private long ticksSinceSongStarted;
    @Nullable
    private RegistryEntry<JukeboxSong> song;
    private final BlockPos pos;
    private final SorrizoJukeboxManager.ChangeNotifier changeNotifier;

    public SorrizoJukeboxManager(SorrizoJukeboxManager.ChangeNotifier changeNotifier, BlockPos pos) {
    ...
    private boolean hasSecondPassed() {
        return this.ticksSinceSongStarted % 10L == 0L;
    }

    private static void spawnNoteParticles(WorldAccess world, BlockPos pos) {
        if (world instanceof ServerWorld serverWorld) {
            Vec3d vec3d = Vec3d.ofBottomCenter(pos).add(0.0F, 1.2F, 0.0F);
            float f = (float)world.getRandom().nextInt(4) / 24.0F;
            int part = world.getRandom().nextInt(3);
            if (part == 0) {
                serverWorld.spawnParticles(ModParticles.SORRIZO1_PARTICLE, vec3d.getX(), vec3d.getY(), vec3d.getZ(), 0, f, 0.0F, 0.0F, 1.0F);
            }
            else if (part == 1){
                serverWorld.spawnParticles(ModParticles.SORRIZO2_PARTICLE, vec3d.getX(), vec3d.getY(), vec3d.getZ(), 0, f, 0.0F, 0.0F, 1.0F);
            }
            else {
                serverWorld.spawnParticles(ModParticles.SORRIZO3_PARTICLE, vec3d.getX(), vec3d.getY(), vec3d.getZ(), 0, f, 0.0F, 0.0F, 1.0F);
            }
        }
